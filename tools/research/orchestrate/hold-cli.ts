 ...
 * (independent-review remediation of WU053). Exactly two actions, both
 * requiring an operator to run this deliberately against a specific cycle
 * directory — never invoked automatically by cli.ts's normal path:
 *
 ...
 *
 *   resume               Resumes an existing pre-Gate HOLD at --cycle-dir:
 *                        reloads the exact frozen manifest/candidates from
 *                        that HOLD (never re-invoking PRIMARY_AUTHOR),
 *                        reruns prospective validation, performs fresh
 *                        Source availability checks, and re-evaluates
 *                        admission. A fresh INDEPENDENT_REVIEWER invocation
 *                        happens only if admission is now ELIGIBLE — see
 *                        run-cycle.ts's resumePreGateHold().
 *
 * Neither action ever performs a canonical write or touches `.aiqt/**`.
 *
 * Usage:
...
import { resumePreGateHold } from "./run-cycle.ts";
...
async function main(): Promise<void> {
  ...
  console.error("Usage: hold-cli.ts <record-resolution|resume> [...flags]");
  process.exitCode = 1;
}
