      ...
      {entries.map((entry) => (
        <li key={`${entry.problemId}-${entry.date}-${entry.summary}`} className="material-change-item">
          ...
          <div className="material-change-content">
            <p className="material-change-problem"><span className="material-change-problem-title">{entry.problemTitle}</span><span className="technical-id material-change-problem-id">{entry.problemId}</span></p>
