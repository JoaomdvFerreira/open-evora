...
- **Production content widths** (supersede the Design System doc-shell width, which is doc chrome only and never production):
  - **Primary desktop outer production frame: 980px remains the canonical maximum**, shared by Header (inner content), Overview, Records, Record Detail, and Problem View via one shared CSS structural primitive (`--shell-content-max` / `.shell-frame`) — not page-specific variants. Record Detail's 980px = 720px main content column + 44px gap (`--detail-gap`) + 216px rail; Problem View reuses that same two-column geometry for its own 980px. The full two-column reading composition (720 main + 44 gap + 216 rail) is used only from `>=1060px`, where enough viewport space exists to preserve the 720px main column without compressing it.
  - **`768px-1059px` intermediate reading recomposition (Record Detail, Problem View only)**: the two-column composition cannot fit in this band without destructively compressing the 720px main column, so these two reading surfaces recompose to a single-column, max-720px reading composition — the rail becomes non-sticky and flows after the main content (Record Detail), or is hidden in favor of the existing compact in-flow section index (Problem View). This reuses the existing compact single-column mechanics rather than a parallel layout system, and does not apply compact typography/navigation rules. This is not a new breakpoint tier for the application generally — `<=767px` remains the sole canonical compact breakpoint (§0.1 above); `768px-1059px` is a narrower CSS-only treatment scoped to these two reading surfaces.
