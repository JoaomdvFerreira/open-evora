...
| Problem reading | `apps/research-explorer/src/problem/ProblemView.tsx` | Page/data-orchestration owner for the public `view=problem` route: data loading, focus entry, and error states. Its presentation body is the approved PRB Details composition (`docs/explorerarchitecture.md` §3.2). Historical snapshot (DS-03A): it rendered Problem header, authored current state, Evidence partition, open questions, contradiction search, investigation path, section index, and rail. |
| Problem history | `apps/research-explorer/src/problem/ProblemHistoryView.tsx` | Authored material history projection with state transitions and Evidence references. It shares the two-view PRB navigation model with ProblemView. Historical snapshot (DS-03A): it reused Problem list/item styles and duplicated the Problem breadcrumb structure. |
| PRB context nav | `apps/research-explorer/src/navigation/ContextTabs.tsx`, `.context-tabs` | Shared PRB domain navigation with exactly two destinations, `Detalhes | Histórico`, used by ProblemView and ProblemHistoryView. It is `<nav>` with current-page semantics, not an ARIA tabs widget. Generic Record Detail and Graph are not destinations. Historical snapshot (DS-03A): it was used by Detail, Problem, History, and dormant Graph context. |
...
| `ContextTabs` | PRB domain; navigate the same PRB identity between `Detalhes` and `Histórico` | Current page only | Remains domain-owned `<nav>` with current-page semantics, not an ARIA tabs widget or the generic navigation primitive. Generic Record Detail and Graph do not consume or appear in it. |
...
- `RecordDetailPanel` retains detail loading/focus entry and type routing, including technical PRB inspection inside Records. It is not a PRB context destination. Its internal domain sections may move to smaller files without becoming shared UI.
- `ProblemView` remains the page/data-orchestration owner for the public `view=problem` route; its presentation body is the approved PRB Details composition, which owns section order and presence. `ProblemHistoryView` remains the authored-history owner and a separate projection rather than a ProblemView variant; both share the two-view PRB navigation.
...
`ContextTabs` stays a PRB-scoped domain component. It is two-destination domain navigation, `Detalhes | Histórico`, preserving one Problem identity across the public reading and its authored history. It uses `<nav>` with current-page semantics, not an ARIA tabs widget, and is not equivalent to global navigation. Generic Record Detail is not a peer PRB context destination; technical PRB inspection remains in `RecordDetailPanel` through Records. Graph does not participate in `ContextTabs`.
...
| `ContextTabs` | `apps/research-explorer/src/navigation/ContextTabs.tsx` | Domain component | PRB-owned, shared by `Detalhes` and `Histórico` | Current page | Prevents PRB navigation drift already | Low; Graph and generic Record Detail are not destinations | **KEEP** |
