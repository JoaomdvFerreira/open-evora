...
### Problem reading and investigation semantics

These clarify meaning only; shape, optionality and field names remain schema-owned. Optional fields are authored only when explicitly supported; absence is not a gap to fill.

- `problem_statement` must not express causal strength beyond what the linked Evidence and its inference limits support.
- `causal_reading` is an authored, bounded Problem-level interpretation. It is not a proven causal claim, and it must not be generated by selecting or ranking an arbitrary subset of the linked Evidence.
- `investigation.open_questions[]`: each field of an entry has distinct meaning.
  - `latest_result` is the current authored result/knowledge for that specific question.
  - `why_open` is why the question remains unresolved; it is not the latest result.
  - `resolution_condition`, when present, is the authored evidential or decision condition required to resolve or reopen the question; it is not the current action.
  - `current_action` is the current investigation activity/action. Free-text tokens in it (for example `WATCH`) carry no structured posture semantics unless a future canonical contract explicitly defines one.
  - `evidence[]` is Evidence related to that question. Its presence does not license stronger wording than that Evidence supports.
- `investigation.path` is an authored narrative of how the investigation reached its current formulation. It is not a progress, completion, current/pending, or other state machine.
- `updated_at` is record-edit metadata, not a currentness assessment.

PRB currentness is an authored judgement against the claim. It must not be inferred solely from `updated_at`, Source dates, Evidence dates, or absence of contradiction.

These clarifications do not change the PRB→EVD `effects[]`/`research_roles[]` semantics in §2 or the material-history contract in §4.
